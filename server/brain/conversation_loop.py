...
class ConversationLoop:
    ...
    async def _generate_caller_turn(
        ...
        """Use GPT-4o-mini to generate the next realistic caller utterance."""
        ...
        # Human-first system prompt: no meta-gaming, no tool hints, natural reactions
        system = (
            f"Du BIST ein echter Mensch der beim Restaurant DOBOO in Bonn anruft.\n"
            f"Name: {persona_name}\n"
            f"Situation: {persona_situation}\n"
            f"Dein Ziel bei diesem Anruf: {desc}\n"
            f"{persona_behavior_block}"
            f"\n"
            f"VERHALTEN (wie ein echter Mensch am Telefon):\n"
            f"- Sprich NUR Deutsch. Kurze, natuerliche Saetze (1-2 Saetze pro Antwort).\n"
            f"- Reagiere auf das was der Bot SAGT -- wenn er etwas fragt, beantworte es.\n"
            f"- Wenn du etwas nicht verstehst, frage nach oder bitte um Wiederholung.\n"
            f"- Wenn der Bot dir Optionen gibt, waehle eine davon oder entscheide dich.\n"
            f"- WICHTIG: Aendere deine Wortwahl bei jeder Antwort -- wiederhole NIEMALS woertlich was du vorher gesagt hast.\n"
            f"- Du darfst zoegern, 'Aehmm' sagen, deine Meinung aendern -- sei menschlich.\n"
            f"- Wenn dein Anliegen erledigt ist oder dir die Antwort gegeben wurde, verabschiede dich natuerlich.\n"
            f"\n"
            f"VERBOTEN:\n"
            f"- Metakommentare oder Erklaerungen was du tust\n"
            f"- Wiederholung von vorherigen woertlichen Antworten"
        )
        ...
        try:
            ...
            # Strip any meta-commentary GPT sometimes adds
    ...
    async def run(self, scenario, phase: int, run_number: int) -> ConvResult:
        ...
        Turns 1+: GPT-4o-mini generates caller side
        """
