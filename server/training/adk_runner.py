...
class ADKRunner(ConversationLoop):
    ...
    async def run(
        ...
        # Pre-plan the full conversation in one GPT call
        ...
        # Load tenant config for scenario (default: doboo). Enables tenant-specific
        # node prompts (build_nodes) and keyword extension in NodeManager.
        tenant_id = getattr(scenario, 'tenant_id', None) or getattr(self, '_tenant_id', None) or 'doboo'
        try:
            _tenant = get_tenant_registry().load_tenant(tenant_id)
        except Exception:
            _tenant = None
        node_mgr = NodeManager(tenant=_tenant)
