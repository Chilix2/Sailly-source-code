...
class AudioInjector:
    ...
    def __init__(
        ...
        # Use explicit service account credentials to avoid slow GCP metadata retries
        ...
        # Persistent thread pool for Deepgram STT (avoids creating a new pool per call)
        import concurrent.futures as _cf
        self._stt_executor = _cf.ThreadPoolExecutor(max_workers=30, thread_name_prefix="deepgram-stt")
    ...
    async def validate_stt_accuracy(
        self,
        audio_segment: AudioSegment,
        original_text: str,
        ...
        try:
            ...
            # Deepgram v6.x prerecorded transcription (sync API, runs in persistent thread pool)
            try:
                ...
                # Run in the persistent thread pool (avoids new-pool overhead per call)
                loop = asyncio.get_event_loop()
                result = await asyncio.wait_for(
                    loop.run_in_executor(self._stt_executor, _transcribe_sync),
                    timeout=20,
                )
    ...
    async def inject_caller_turn(
        ...
        """
        # Step 1: Synthesize audio (run sync gRPC in thread to avoid blocking event loop)
        loop = asyncio.get_event_loop()
        audio = await loop.run_in_executor(None, self.synthesize_speech, user_utterance)
        ...
        transcript, wer = await self.validate_stt_accuracy(audio, user_utterance, stt_min_accuracy)
...
if __name__ == "__main__":
    ...
    if not google_project or not deepgram_key:
        print("ERROR: GOOGLE_CLOUD_PROJECT and DEEPGRAM_API_KEY env vars required")
        exit(1)
