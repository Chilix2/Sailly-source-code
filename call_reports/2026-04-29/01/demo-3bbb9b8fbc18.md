...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-3bbb9b8fbc18` |
| **Caller / channel** | `browser_demo` |
| **Started** | 2026-04-29 01:08:37 UTC |
| **Ended** | 2026-04-29 01:09:34 UTC |
| **Duration** | **57 seconds (0:57 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_disconnect` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 1567 ms |
| **p95 latency (stored)** | 1567 ms |
| **Max latency (stored)** | 1567 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 81.7421 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 1567 / 1567 / 1567 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 1564 / 1564 ms | From `llm_latency_ms` column |
...
stt_ms / extract_ms / l2_ms / tool_ms / tts_first_byte_ms: None / 1544 / None / 18 / None
...
**User:** Hallo, Selly. Ich wollte nur kurz quatschen. Wie gehen's dir so?

**Bot:** Danke der Nachfrage — mir geht's prima, ich bin bereit für die beste Bestellung des Tages! Aber sag mal, hast du Hunger auf koreanisches Soulfood, oder rufst du einfach so an zum Plaudern?
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 1567 | 1564 | 3 | None | — | 200 | greeting_first | neutral | None | get_menu,ai_greeting |  |
...
- **user** (turn 1): Hallo, Selly. Ich wollte nur kurz quatschen. Wie gehen's dir so?
