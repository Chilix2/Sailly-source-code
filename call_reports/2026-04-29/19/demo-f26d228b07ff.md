...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-f26d228b07ff` |
| **Caller / channel** | `browser_demo` |
| **Started** | 2026-04-29 19:11:22 UTC |
| **Ended** | 2026-04-29 19:12:21 UTC |
| **Duration** | **59 seconds (0:59 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_disconnect` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 1878 ms |
| **p95 latency (stored)** | 1878 ms |
| **Max latency (stored)** | 1878 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 84.6858 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 1878 / 1878 / 1878 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 1874 / 1874 ms | From `llm_latency_ms` column |
...
stt_ms / extract_ms / l2_ms / tool_ms / tts_first_byte_ms: None / 1844 / None / 26 / None
...
**User:** Hallo, Sadie. Ich wollte nur kurz quatschen. Wie geht's dir denn heute so?

**Bot:** Danke der Nachfrage — mir geht's prima, und hier im DOBOO duftet's heute wieder herrlich nach Kimchi und Bulgogi! Aber sagen Sie, was kann ich für Sie tun — möchten Sie bestellen oder haben Sie eine Frage zum Restaurant?
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 1878 | 1874 | 4 | None | — | 200 | greeting_first | neutral | None | get_date_info,get_menu,ai_greeting |  |
...
- **user** (turn 1): Hallo, Sadie. Ich wollte nur kurz quatschen. Wie geht's dir denn heute so?
