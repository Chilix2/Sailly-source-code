...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-dc09d37509fb` |
| **Caller / channel** | `[validation:A2_tool_tag_wuetend]` |
| **Started** | 2026-05-01 06:55:19 UTC |
| **Ended** | 2026-05-01 06:55:31 UTC |
| **Duration** | **12 seconds (0:12 min)** |
| **Total DB turns** | 1 |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 11 / 11 / 11 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 7 / 7 ms | From `llm_latency_ms` column |
| STT p50 | 4 ms | From `stt_latency_ms` |
| Loop incidents | 0 | `loop_detected_in_stream` |
| Barge-in successes | 0 | `barge_in_succeeded` |
| Error codes (distinct) | — | `error_codes` |
| **[Phase 3] TTS latency** p50 / max | 7 / 7 ms (1 turns) | From `tts_latency_ms` |
...
**User:** Das sagst Du ja jedes Mal, dass ihr in Bonn seid, aber ich will die genaue Adresse.
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 11 | 7 | 4 | None | — | 200 | info_neutral | neutral | None | get_menu |  |

---

## 6. Transcripts (`google_transcripts`)

- **user** (turn 0): Ja, hallo, ich bin total genervt, weil ich schon wieder nicht weiß, wo ihr eigentlich seid. Wo befindet sich euer Restaurant genau?
- **user** (turn 1): Das sagst Du ja jedes Mal, dass ihr in Bonn seid, aber ich will die genaue Adresse.
