...
| **p95 latency (stored)** | 224 ms |
| **Max latency (stored)** | 224 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 54.2017 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 224 / 224 / 224 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 218 / 218 ms | From `llm_latency_ms` column |
| STT p50 | 5 ms | From `stt_latency_ms` |
| Loop incidents | 0 | `loop_detected_in_stream` |
| Barge-in successes | 0 | `barge_in_succeeded` |
| Error codes (distinct) | — | `error_codes` |
| **[Phase 3] TTS latency** p50 / max | 219 / 219 ms (1 turns) | From `tts_latency_ms` |
...
**User:** Ich möchte zweimal Burgogi zum Abholen bestellen. Mein Name ist Schmidt, Telefon null sieben neun sieben acht vier vier sechs sieben acht neun. Können Sie das aufnehmen?
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 224 | 218 | 5 | None | — | 200 | greeting_first | neutral | None | get_menu |  |

---

## 6. Transcripts (`google_transcripts`)

- **user** (turn 0): Ich möchte zweimal Burgogi zum Abholen bestellen. Mein Name ist Schmidt, Telefon null sieben neun sieben acht vier vier sechs sieben acht neun. Können Sie das aufnehmen?
