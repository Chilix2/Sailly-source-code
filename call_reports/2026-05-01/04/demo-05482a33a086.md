...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-05482a33a086` |
| **Caller / channel** | `[validation:A4_basic_order_wuetend]` |
| **Started** | 2026-05-01 04:40:36 UTC |
| **Ended** | 2026-05-01 04:41:12 UTC |
| **Duration** | **36 seconds (0:36 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_hangup` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 365 ms |
| **p95 latency (stored)** | 365 ms |
| **Max latency (stored)** | 365 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 51.354 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 365 / 365 / 365 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 361 / 361 ms | From `llm_latency_ms` column |
| STT p50 | 3 ms | From `stt_latency_ms` |
| Loop incidents | 0 | `loop_detected_in_stream` |
| Barge-in successes | 0 | `barge_in_succeeded` |
| Error codes (distinct) | — | `error_codes` |
| **[Phase 3] TTS latency** p50 / max | 362 / 362 ms (1 turns) | From `tts_latency_ms` |
...
**User:** Rufen Sie mich bitte zurück. Wenn Sie meine Bestellung aufgenommen haben.
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 365 | 361 | 3 | None | — | 200 | greeting_first | neutral | None | get_menu |  |

---

## 6. Transcripts (`google_transcripts`)

- **user** (turn 0): Rufen Sie mich bitte zurück. Wenn Sie meine Bestellung aufgenommen haben.
