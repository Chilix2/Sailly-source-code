...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-f2410309fcbb` |
| **Caller / channel** | `[validation:A4_basic_order_aeltere_person]` |
| **Started** | 2026-05-01 04:43:17 UTC |
| **Ended** | 2026-05-01 04:43:57 UTC |
| **Duration** | **39 seconds (0:39 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_hangup` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 408 ms |
| **p95 latency (stored)** | 408 ms |
| **Max latency (stored)** | 408 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 55.7095 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 408 / 408 / 408 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 377 / 377 ms | From `llm_latency_ms` column |
| STT p50 | 31 ms | From `stt_latency_ms` |
| Loop incidents | 0 | `loop_detected_in_stream` |
| Barge-in successes | 0 | `barge_in_succeeded` |
| Error codes (distinct) | — | `error_codes` |
| **[Phase 3] TTS latency** p50 / max | 377 / 377 ms (1 turns) | From `tts_latency_ms` |
...
worker_profile:    —
stt_confidence:    None
tools_called:      ['get_menu', 'create_order']
...
**User:** Ich hätte gerne zweimal Bulgogi zum Abholen. Mein Name ist Schmidt und die Telefonnummer ist null eins neun fünf fünf fünf sechs acht neun neun. Haben Sie das verstanden?

**Bot:** Ich habe Ihre Bestellung für Sie aufgenommen. Stimmt das so?
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 408 | 377 | 31 | None | — | 200 | greeting_first | neutral | None | get_menu,create_order |  |

---

## 6. Transcripts (`google_transcripts`)

- **user** (turn 0): Ich hätte gerne zweimal Bulgogi zum Abholen. Mein Name ist Schmidt und die Telefonnummer ist null eins neun fünf fünf fünf sechs acht neun neun. Haben Sie das verstanden?
- **assistant** (turn 1): Ich habe Ihre Bestellung für Sie aufgenommen. Stimmt das so?
