...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-4eab1b88e7de` |
| **Caller / channel** | `[validation:A4_basic_order_aeltere_person]` |
| **Started** | 2026-05-01 04:45:18 UTC |
| **Ended** | 2026-05-01 04:45:52 UTC |
| **Duration** | **34 seconds (0:34 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_hangup` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 267 ms |
| **p95 latency (stored)** | 267 ms |
| **Max latency (stored)** | 267 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 48.5063 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 267 / 267 / 267 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 219 / 219 ms | From `llm_latency_ms` column |
| STT p50 | 48 ms | From `stt_latency_ms` |
| Loop incidents | 0 | `loop_detected_in_stream` |
| Barge-in successes | 0 | `barge_in_succeeded` |
| Error codes (distinct) | — | `error_codes` |
| **[Phase 3] TTS latency** p50 / max | 220 / 220 ms (1 turns) | From `tts_latency_ms` |
...
**User:** Ich möchte zweimal Burgogi zum Abholen bestellen. Mein Name ist Schmidt und die Telefonnummer ist null sieben neun drei vier vier sechs sechs acht neun neun.
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 267 | 219 | 48 | None | — | 200 | greeting_first | neutral | None | get_menu |  |

---

## 6. Transcripts (`google_transcripts`)

- **user** (turn 0): Ich möchte zweimal Burgogi zum Abholen bestellen. Mein Name ist Schmidt und die Telefonnummer ist null sieben neun drei vier vier sechs sechs acht neun neun.
