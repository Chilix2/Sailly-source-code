...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-be5fc03a4355` |
| **Caller / channel** | `browser_demo` |
| **Started** | 2026-04-28 22:24:03 UTC |
| **Ended** | 2026-04-28 22:24:54 UTC |
| **Duration** | **50 seconds (0:50 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_hangup` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 1902 ms |
| **p95 latency (stored)** | 1902 ms |
| **Max latency (stored)** | 1902 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 72.0163 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 1902 / 1902 / 1902 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 1898 / 1898 ms | From `llm_latency_ms` column |
...
llm_latency_ms:    1898 ✅
tts_latency_ms:    0
total_latency_ms:  1902 ✅
stt_ms / extract_ms / l2_ms / tool_ms / tts_first_byte_ms: None / 1878 / None / 18 / None
...
worker_profile:    —
stt_confidence:    None
tools_called:      ['get_directions', 'get_menu', 'ai_greeting']
...
**User:** Hallo, Sally. Wie komme ich vom Hauptbahnhof am besten zu euch?

**Bot:** Vom Hauptbahnhof aus nehmen Sie am besten die Straßenbahn Linie 16 oder 63 Richtung Stadtmitte — Ausstieg Juridicum, dann sind Sie in etwa 10 Minuten bei uns in der Friedrich-Ebert-Allee 69. Alternativ ist es auch ein gemütlicher 15-Minuten-Spaziergang durchs Zentrum.
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 1902 | 1898 | 3 | None | — | 200 | greeting_first | neutral | None | get_directions,get_menu,ai_greeting |  |
...
- **user** (turn 1): Hallo, Sally. Wie komme ich vom Hauptbahnhof am besten zu euch?
