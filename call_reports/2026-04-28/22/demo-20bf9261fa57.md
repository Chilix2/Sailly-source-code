...
| Parameter | Value |
|-----------|-------|
| **Call SID** | `demo-20bf9261fa57` |
| **Caller / channel** | `browser_demo` |
| **Started** | 2026-04-28 22:25:29 UTC |
| **Ended** | 2026-04-28 22:26:30 UTC |
| **Duration** | **60 seconds (1:00 min)** |
| **Total DB turns** | 1 |
| **Outcome / reason** | `client_disconnect` |
| **Quality score** | 5.0 / 10 |
| **Avg latency (stored)** | 1688 ms |
| **p95 latency (stored)** | 1688 ms |
| **Max latency (stored)** | 1688 ms |
| **Was escalated** | False |
| **Tenant** | `—` |
| **Est. cost (stored)** | 86.1071 ¢ |
...
| Check | Value | Notes |
|-------|-------|-------|
| Latency p50 / p95 / max (total) | 1688 / 1688 / 1688 ms | Alert threshold env `MONITOR_LATENCY_P95_MS=3000` |
| LLM span (approx) p50 / max | 1685 / 1685 ms | From `llm_latency_ms` column |
...
stt_ms / extract_ms / l2_ms / tool_ms / tts_first_byte_ms: None / 1668 / None / 15 / None
...
**User:** Velle, Achtung, Du hast schon wieder den falschen Namen gesagt. Es heißt Dobo, nicht Duvu. Das ist schon mehrmals vorgekommen.

**Bot:** Das tut mir sehr leid — Sie haben vollkommen recht, und ich entschuldige mich für die Verwirrung. Es ist DOBOO, nicht Duvu — danke, dass Sie mich korrigieren. Das sollte nicht vorkommen. Wie kann ich Ihnen jetzt helfen?
...
|------|----------|--------|--------|----------|--------|--------------|-----------|------|-----|-------|-------|
| 1 | 1688 | 1685 | 3 | None | — | 200 | greeting_first | neutral | None | get_menu,ai_greeting |  |
...
- **user** (turn 1): Velle, Achtung, Du hast schon wieder den falschen Namen gesagt. Es heißt Dobo, nicht Duvu. Das ist schon mehrmals vorgekommen.
