...
export function AudioPlayerButton() {
    ...
    return (<button onClick={isPlaying ? pause : play} className="p-2 hover:bg-zinc-800 rounded-md transition-colors">
      {isPlaying ? <Pause size={20}/> : <Play size={20}/>}
    </button>);
}
export function AudioPlayerProgress() {
    const { currentTime, duration, seek } = useAudioPlayer();
    return (<input type="range" min="0" max={duration} value={currentTime} onChange={(e) => seek(parseFloat(e.target.value))} className="flex-1 h-1 bg-zinc-800 rounded-lg appearance-none cursor-pointer" style={{
...
export function AudioPlayerSpeed() {
    ...
    return (<div className="relative inline-block group">
      <button className="p-2 hover:bg-zinc-800 rounded-md transition-colors flex items-center gap-1 text-sm">
        <Settings size={16}/>
        {playbackRate}x
      </button>
      <div className="absolute hidden group-hover:block right-0 bg-zinc-900 border border-zinc-800 rounded-md shadow-lg z-10">
        {speeds.map((speed) => (<button key={speed} onClick={() => setPlaybackRate(speed)} className={`block w-full text-left px-4 py-2 text-sm hover:bg-zinc-800 transition-colors ${playbackRate === speed ? 'bg-zinc-800 text-accent' : ''}`}>
            {speed}x
          </button>))}
      </div>
    </div>);
