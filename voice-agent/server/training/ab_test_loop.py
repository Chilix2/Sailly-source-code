...
def _generate_validation_reports(output_dir: Path) -> None:
    """Generate SUMMARY_REPORT.json and CONSOLIDATED_FAILURE_REPORT.json after validation run."""
    from collections import defaultdict
    
    try:
        results_file = output_dir / "ab_results.json"
        ...
        passes = [r for r in results if r.get("one_live_pass")]
        fails = [r for r in results if not r.get("one_live_pass")]
        ...
        failure_report = {
            ...
            "summary": {
                ...
                "passed": len(passes),
                "failed": len(fails),
                "pass_rate": f"{len(passes) * 100 / len(results):.1f}%" if results else "0%",
            },
            "failures_by_cluster": {},
            "all_failures": [],
        }
        ...
        for r in fails:
            ...
            root = missing[0] if missing else "score_only"
        ...
        for root_cause, items in sorted(clusters.items(), key=lambda x: -len(x[1])):
            ...
            for r in items:
                cluster_data["scenarios"].append({
                    "scenario_id": r["scenario_id"],
                    "phase": r.get("phase"),
                    "category": r.get("category"),
                    "expected_tools": r.get("expected_tools", []),
                    "actual_tools": r.get("one_live_tools", []),
                    "missing_tools": r.get("one_live_missing_tools", []),
                    "composite_score": r.get("one_live_composite"),
                    "turns": r.get("one_live_turns"),
                    "end_reason": r.get("one_live_end_reason"),
                    "failures": r.get("one_live_failures", []),
                })
                failure_report["all_failures"].append(cluster_data["scenarios"][-1])
            failure_report["failures_by_cluster"][root_cause] = cluster_data
        
        # Write consolidated failure report
        with open(output_dir / "CONSOLIDATED_FAILURE_REPORT.json", "w") as f:
        ...
        summary = {
            ...
            "top_failures": _bucket_failures(fails) if fails else [],
        }
        ...
        logger.info(f"Generated CONSOLIDATED_FAILURE_REPORT.json and SUMMARY_REPORT.json in {output_dir}")
...
class ABTestLoop:
    ...
    def _make_runner_stack(self) -> RunnerStack:
        ...
        one_live_loop._tenant_id = self.tenant_id
        return RunnerStack(
    ...
    def _load_scenarios(self, scenario_ids: Optional[List[str]] = None):
        ...
        if self.phases:
            ...
            # Filter to specific IDs if provided (e.g. targeted re-run of known failures)
            if scenario_ids:
                id_set = set(scenario_ids)
                scenarios = [s for s in scenarios if s.id in id_set]
                logger.info(
                    f"  Filtered to {len(scenarios)} scenarios matching --ids "
                    f"({len(scenario_ids)} requested)"
                )
    ...
    async def run(self, scenario_ids: Optional[List[str]] = None):
        ...
        # Generate consolidated failure report and summary
        _generate_validation_reports(self.output_dir)
...
async def main():
    ...
    parser.add_argument(
        "--tenant-id",
        default=os.environ.get("TENANT_ID", "doboo"),
        help=(
            "Tenant configuration to use (default: doboo). "
            "Can also be set via TENANT_ID env var. "
            "Example: --tenant-id doboo"
        ),
    )
    ...
    ab = ABTestLoop(
        ...
        tenant_id=getattr(args, 'tenant_id', 'doboo'),
    )
